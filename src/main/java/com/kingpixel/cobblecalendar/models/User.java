...
public class User implements Serializable {
  ...
  public void onJoin(ServerPlayerEntity player) {
    ...
    if (hasMissedDays() && CobbleCalendar.config.isAutoReset()) {
      reset(player, false);
      return;
    }

    if (day >= CobbleCalendar.config.maxDay()) {
      reset(player, true);
      return;
    }
  ...
  public void reset(ServerPlayerEntity player, boolean completed) {
    ...
    String message = completed
      ? CobbleCalendar.language.getMessageCompleted()
      : CobbleCalendar.language.getMessageReset();
  ...
  public void fix() {
    ...
    if (day > CobbleCalendar.config.maxDay()) {
      day = CobbleCalendar.config.maxDay();
      changed = true;
    }
  ...
  public CompletableFuture<Void> save() {
    if (getDirty().compareAndSet(true, false)) return CobbleCalendar.database.saveOrUpdateUser(this);
